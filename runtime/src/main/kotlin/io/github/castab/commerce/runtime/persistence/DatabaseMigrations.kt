...
/**
 * Applies the Flyway migrations of commerce-runtime, then those of the application.
 *
 * The two sets evolve independently, so each has its own history table:
 *
 * - commerce-runtime migrations are discovered at [COMMERCE_LOCATION] and own the
...
class DatabaseMigrations(
    ...
    companion object {
        /** The PostgreSQL schema that holds every commerce-runtime table. */
        const val COMMERCE_SCHEMA = "commerce"

        /** Where commerce-runtime's own migrations are discovered. Applications must not use it. */

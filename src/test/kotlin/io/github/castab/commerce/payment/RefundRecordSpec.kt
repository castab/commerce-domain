...
class RefundRecordSpec : FunSpec({
    ...
    test("references the original payment by its UUID, not a financial document") {
        ...
        RefundRecord::class.java.declaredFields.map { it.type.simpleName }.toSet() shouldBe
    ...
    test("a positive refund is accepted, up to the full payment amount") {
        RefundRecord.create(UUID.randomUUID(), payment, usd("0.01"), PaymentMethod.CARD, minutesLater(1))
            .amount shouldBe usd("0.01")
        RefundRecord.create(UUID.randomUUID(), payment, usd("500"), PaymentMethod.CARD, minutesLater(1))
    ...
    test("multiple partial refunds are supported while their total stays within the payment") {
        val refunds = listOf("200.00", "200.00", "100.00").map { amount ->
            RefundRecord.create(UUID.randomUUID(), payment, usd(amount), PaymentMethod.CARD, minutesLater(1))
        }
    ...
    test("cumulative refunds above the payment are rejected during reconciliation") {
        val refunds = listOf("300.00", "200.01").map { amount ->
            RefundRecord.create(UUID.randomUUID(), payment, usd(amount), PaymentMethod.CARD, minutesLater(1))
        }
    ...
    test("the refund method may differ from the payment method") {
        ...
        RefundRecord.create(UUID.randomUUID(), debit, usd("25.00"), PaymentMethod.CASH, minutesLater(1))
            .method shouldBe PaymentMethod.CASH

        PaymentReconciliation.reconcile(check, emptyList(), refunds = listOf(cashRefund))
    ...
    test("the external refund reference is optional") {
        ...
        val processed = RefundRecord.create(
            UUID.randomUUID(),
            payment,
            usd("10"),
            PaymentMethod.CARD,
    ...
    test("restore rebuilds an equal refund") {
        val created = RefundRecord.create(
            UUID.randomUUID(),
            payment,
            usd("42.00"),
            PaymentMethod.CARD,

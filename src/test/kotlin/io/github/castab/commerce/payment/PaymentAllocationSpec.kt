...
class PaymentAllocationSpec : FunSpec({
    ...
    context("references") {
        ...
        test("exact-snapshot and lineage queries are both plain comparisons") {
            ...
            allocations.filter { it.financialDocumentReference == quoteV2.reference }
                .map { it.amount } shouldContainExactly listOf(usd("200"))
            allocations.filter { it.financialDocumentReference.id == quoteV2.id }
                .map { it.amount } shouldContainExactly listOf(usd("100"), usd("200"))
        }

        test("retains neither the payment nor the document") {
            PaymentAllocation::class.java.declaredFields.map { it.type.simpleName }.toSet() shouldBe
    ...
    context("currency") {
        ...
        test("matching payment and document currencies succeed") {
            ...
            PaymentAllocation.create(UUID.randomUUID(), payment, invoice, usd("500.00"), minutesLater(1))
    ...
    context("amount") {
        ...
        test("the full payment amount may be allocated, compared numerically") {
            PaymentAllocation.create(UUID.randomUUID(), payment, invoice, usd("500"), minutesLater(1))
    ...
    context("restoration") {
        ...
        test("restore rebuilds an equal allocation from references alone") {
            ...
            val restored = PaymentAllocation.restore(
                id = created.id,

...
class BookingModelsSpec : FunSpec({
    ...
    test("separate bookings reference one customer by identity without embedding it") {
        ...
        Booking::class.java.declaredFields.map { it.name }.filterNot { it.startsWith("$") }.toSet() shouldBe
            setOf("id", "customerId")
    }
    ...
    test("customer-backed contact references the customer without duplicating their details") {
        ...
        BookingContact.CustomerContact::class.java.declaredFields.map { it.name }.filterNot { it.startsWith("$") }.toSet() shouldBe
    ...
    test("multiple independently identified contacts may serve one booking") {
        ...
        val other = BookingContact.ExternalContact(
            BookingContactId(UUID.randomUUID()), first.id, CustomerName("Alice Smith"),
            email = EmailAddress("alice@example.com"),
        )
        val dayOf = BookingContact.ExternalContact(
            BookingContactId(UUID.randomUUID()), first.id, CustomerName("Venue representative"),
        ...
        BookingContact.ExternalContact::class.java.declaredFields.map { it.name }.filterNot { it.startsWith("$") }.contains("customerId") shouldBe false
    }
    ...
    test("external contact requires a name and at least one contact method") {
        ...
        val valid = BookingContact.ExternalContact(
            BookingContactId(UUID.randomUUID()), first.id, CustomerName("Alice"), phoneNumber = PhoneNumber("555-2222"),
        )
    ...
    test("locations belong to bookings, and the same customer may have different addresses") {
        val firstLocation = BookingLocation(
            BookingLocationId(UUID.randomUUID()), first.id,
            PostalAddress("123 Main St", "Suite 2", "Fresno", "CA", "93721", "US"),
        )
        val secondLocation = BookingLocation(
            BookingLocationId(UUID.randomUUID()), second.id,
        ...
        BookingLocation::class.java.declaredFields.map { it.name }.filterNot { it.startsWith("$") }.toSet() shouldBe
    ...
    test("address fields reject blanks and operational records can be discarded independently") {
        ...
        val contacts = mutableListOf<BookingContact>(BookingContact.CustomerContact(BookingContactId(UUID.randomUUID()), first.id, customer.id))
        val locations = mutableListOf(BookingLocation(BookingLocationId(UUID.randomUUID()), first.id, PostalAddress("123 Main", city = "Fresno", region = "CA", postalCode = "93721")))
        contacts.clear()
        locations.clear()

package io.github.castab.commerce.service.testing
 ...
 * container started by service/build.gradle.kts, or TEST_DATABASE_JDBC_URL).
 *
...
class TestDatabase private constructor(
    ...
    val configuration: CommerceServiceConfiguration.Database,
) : AutoCloseable {
    override fun close() {
    ...
    companion object {
        private fun property(name: String): String =
            System.getProperty("commerce.test.database.$name")
                ?: error("commerce.test.database.$name is not set; run the service tests through Gradle")
        ...
        fun create(): TestDatabase {
            ...
            return TestDatabase(
                name,
                CommerceServiceConfiguration.Database(
                    jdbcUrl = jdbcUrl,
                    username = username,
                    password = password,
                    maximumPoolSize = 4,

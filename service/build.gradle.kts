...
// commerce-service: the opinionated, reusable commerce application/runtime layer.
//
// It depends on :domain (never the reverse) and carries the service runtime stack:
...
base {
    archivesName = "commerce-service"
}

// A development entry point only (`./gradlew :service:run`). Concrete deployments are
// consuming applications that call commerceService(...) from their own main.
application {
    mainClass = "io.github.castab.commerce.service.runtime.DevelopmentServerKt"
}
...
dependencies {
    ...
    // Types from these libraries appear in the public service API (routes, handlers,
...
publishing {
    publications {
        create<MavenPublication>("maven") {
            artifactId = "commerce-service"
            from(components["java"])

            pom {
                name = "commerce-service"
                description = "Opinionated, reusable commerce application runtime built on " +
